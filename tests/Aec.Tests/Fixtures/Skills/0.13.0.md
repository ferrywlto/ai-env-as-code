---
name: aec
description: Operate AI Environment as Code through explicit init, status, backup, apply, version, and installed-skill upgrade flows. Use when the user invokes `$aec`, names an `aec` command, asks to initialize, inspect, back up, apply, or upgrade AEC, or requests a change to personal Codex instructions or configuration managed by AEC.
---
...
success, continue normally. Do not require an app restart or a new task; the next
request that selects `$aec` reads the updated installed guidance.
